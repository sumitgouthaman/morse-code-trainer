...
function nextMorseToChar(morseToChar) {
    morseToChar.correctCharacter = morseToChar.practiceMode.getRandomCharacter();
...
function handleMorseGuess(guess, morseToChar) {
    ...
    morseToChar.practiceMode.recordAttemptAndCheckToast(morseToChar.correctCharacter, isCorrect);

    if (isCorrect) {
        morseToChar.practiceMode.applyFeedback(morseToChar.morseDisplay, 'correct', () => {
            nextMorseToChar(morseToChar);
        });
    } else {
        morseToChar.practiceMode.applyFeedback(morseToChar.morseDisplay, 'incorrect', () => {
