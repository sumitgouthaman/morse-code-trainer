...
- Injected before page navigation using `evaluateOnNewDocument()`
- Uses predetermined values that map to specific characters in the app's character array
- Preserves original Math.random function for potential restoration
- Cycles through values predictably for multiple random calls
- Works across all practice modes (char-to-morse, morse-to-char, sound-to-char)
...
The screenshot system includes a Math.random override to ensure consistent character display across runs. This prevents documentation churn and maintains professional appearance.

**Monitoring**: If you notice characters changing between screenshot runs, verify that:
- The `mockRandomValues` array contains appropriate values (0.0-1.0 range)
- Values map correctly to your app's character selection logic
- The override is being applied before navigation (`evaluateOnNewDocument`)

**Customization**: To change which characters appear in screenshots:
1. Identify the character index you want (A=0, B=1, etc.)
2. Calculate the corresponding random value (index / total_characters)
3. Update the `mockRandomValues` array with the new value
4. Test with a single screenshot run to verify

**Debugging Random Issues**:
- Check browser console for JavaScript errors during character selection
- Verify the app's character array hasn't changed structure
- Ensure the override is applied to fresh pages (not reused browser instances)
